...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 transition-all duration-300 pointer-events-none">
      ...
      {!isScrolled && (
        <div className={`w-full py-1.5 px-6 hidden md:flex items-center justify-between text-[11px] font-mono pointer-events-auto transition-colors duration-300 ${
          isTransparent
      ...
      <div className={`w-full transition-all duration-500 ease-out px-4 sm:px-6 ${
        isScrolled ? 'pt-2.5 pb-2' : 'pt-3 pb-3'
      }`}>
        <div className={`mx-auto transition-all duration-500 pointer-events-auto flex items-center justify-between ${
          isScrolled
            ? 'max-w-6xl bg-white/95 backdrop-blur-xl border border-slate-200 rounded-full px-5 py-2 shadow-xl shadow-blue-950/10'
            : isTransparent
          ...
          >
            <MkrdLogo size="md" theme={isTransparent ? 'dark' : 'light'} />
          </button>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-full transition-all flex items-center gap-1.5 relative ${
                    isTransparent
                      ? isActive
                        ? 'text-white bg-white/20 border border-white/40 font-bold shadow-sm backdrop-blur-md'
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="px-4 py-2 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold font-sans flex items-center gap-1.5 transition-all shadow-md shadow-blue-600/30 active:scale-95 border border-blue-400/30"
            >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-4 top-20 z-50 bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl shadow-blue-950/20 space-y-5 animate-in fade-in duration-200 pointer-events-auto">
          ...
          <div className="pt-4 border-t border-slate-100 space-y-2">
            ...
            <a
              ...
              className="w-full py-2.5 rounded-xl bg-slate-100 text-slate-800 font-semibold text-xs flex items-center justify-center gap-2"
            >
